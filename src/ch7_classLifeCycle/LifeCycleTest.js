...
class LifeCycleTest extends Component {
  // 생명주기를 알아보기 위한 예제코드
  //리액트 다루는 기술 저자 김민준 velopert 검색하면 관련문서 엄청많고 정리도 깔끔하다 .
  ...
  //useRef처럼, 특정의 태그 요소에 접근하기 위한 도구로 사용.
  ...
  constructor(props) {
    ...
    //부모를 초기화 해주기
    super(props);
    console.log("1번째 호출 : constructor 호출");
  }
  ...
  //4번째 호출 : 마운트가 완료 되었어요
  componentDidMount() {
    console.log("4번째 호출 : componentDidMount호출 마운트가 완료 되었어요");
  }
  ...
  shouldComponentUpdate(nextProps, nextState) {
    console.log(
      "3번째 호출 1: shouldComponentUpdate 호출",
      nextProps,
      nextState
    );
    //숫자가 일의자리가 4이면 리렌더링 안하겠다.
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    console.log(
      "5번째 호출  : componentDidUpdate 업데이트 완료",
      prevProps,
      prevState
    );
    if (snapshot) {
  ...
  //3번째 호출2 : 마운트 트리, 화면에 그려주는 역할 함수
  render() {
    console.log("순서 3-2번째 render 호출 : 마운트 또는 업데이트 시 호출");
    ...
    return (
      <div>
        ...
        <br />
        <br />
        <Button title="메인화면으로 이동" type="primary">
          메인 화면으로 이동
        </Button>
      </div>
    );
