...
const TestZone = () => {
  ...
  const nextState2 = produce(array, (draft) => {
    draft.class = { name: "kkt" };
  });

  //데이터 추가2
  const nextState3 = produce(sampleData, (draft) => {
    draft.push({ id: 999, name: "kkt2", age: "33" });
  });
  ...
  console.log("nextState produce ( 원본 데이터, 변경할 업데이트 함수)");
  console.log(nextState);
  console.log("nextState2 name:kkt 클래스 추가");
  console.log(nextState2);
  console.log("nextState3 샘플 데이터3개 + other 추가");
  console.log(nextState3);
  // console.log("nextState4 데이터 제거1");
  // console.log(nextState4);
  console.log("nextState5 splice 함수");
